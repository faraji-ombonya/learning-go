...
// Rate limiting is an important mechanism for controlling resourse utilization
// and maintaining quality of service. Go elegantly supports rate limiting with
// Go routines, channels and tickers.
...
func main() {
	...
	// handling of incoming requests. We'll serve thse requests off a channel
	// of the same name
	...
	// scheme while preserving the overal rate limit, We can accomplish this
	// by buffering our limiter channel. This burstyLimiter channel will
	...
	// Every 200 milliseconds we'll try to add a new value to the
...
// For the second batch of requests we server the first 3 immediately because
// of the burstable rate limiting, then serve the remaining 2 with ~200ms
// delays each.
